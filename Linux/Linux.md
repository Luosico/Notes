...
```
链接：目录中指向文件真实位置的占位符，分为软链接和硬链接

软链接：ln -s origin_file file
		就是实实在在的文件，指向存放在虚拟目录结构中某个地方的另一个文件的地址，他们彼此内容不相同，inode编号和文件大小也不相同，相当于存放地址；可以链接目录或不存在的文件
		
硬链接： ln  origin_file  file
		会创建独立的虚拟文件，包含了原始文件的信息及位置，但它们从根本上而言是同一个件,inode编号和文件大小完全相同
		引用硬链接文件等同于引用源文件；创建硬链接，原始文件也必须实现存在，相当于指向文件系统的同一个文件
